  ...
  try {
    ...
    if (bedrooms) {
      ...
      if (!isNaN(bedroomsNum)) {
        const bedroomsConditions: any[] = [];
        if (bedroomsNum === 3) {
          // 3+ bedrooms - try numeric comparison
          bedroomsConditions.push(
            { $expr: { $gte: [{ $toInt: { $ifNull: ["$amenities.beds", "0"] } }, 3] } },
            { $expr: { $gte: [{ $toInt: { $ifNull: ["$amenities.Bedrooms", "0"] } }, 3] } },
            { $expr: { $gte: [{ $toInt: { $ifNull: ["$amenities.bedrooms", "0"] } }, 3] } },
            { "amenities.beds": { $gte: 3 } },
            { "amenities.Bedrooms": { $gte: 3 } },
            { "amenities.bedrooms": { $gte: 3 } }
          );
        } else {
          // Exact match - try both string and number
          bedroomsConditions.push(
            { "amenities.beds": bedroomsNum },
            { "amenities.beds": String(bedroomsNum) },
            { "amenities.Bedrooms": bedroomsNum },
            { "amenities.Bedrooms": String(bedroomsNum) },
            { "amenities.bedrooms": bedroomsNum },
            { "amenities.bedrooms": String(bedroomsNum) }
          );
        }
    ...
    if (bathrooms) {
      const bathroomsNum = parseInt(bathrooms as string);
      if (!isNaN(bathroomsNum)) {
        const bathroomsConditions: any[] = [];
        if (bathroomsNum === 3) {
          // 3+ bathrooms
          bathroomsConditions.push(
            { $expr: { $gte: [{ $toInt: { $ifNull: ["$amenities.bathrooms", "0"] } }, 3] } },
            { $expr: { $gte: [{ $toInt: { $ifNull: ["$amenities.Bathrooms", "0"] } }, 3] } },
            { "amenities.bathrooms": { $gte: 3 } },
            { "amenities.Bathrooms": { $gte: 3 } },
            { "amenities.bathroom": { $gte: 3 } },
            { "amenities.Bathroom": { $gte: 3 } }
          );
        } else {
          // Exact match
          bathroomsConditions.push(
            { "amenities.bathrooms": bathroomsNum },
            { "amenities.bathrooms": String(bathroomsNum) },
            { "amenities.Bathrooms": bathroomsNum },
            { "amenities.Bathrooms": String(bathroomsNum) },
            { "amenities.bathroom": bathroomsNum },
            { "amenities.bathroom": String(bathroomsNum) },
            { "amenities.Bathroom": bathroomsNum },
            { "amenities.Bathroom": String(bathroomsNum) }
          );
        }
